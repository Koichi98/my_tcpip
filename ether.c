...
/* 
    Create the ethernet frame and execute the callback function given as an argument.
    Since the frame creation is common to all Ethernet devices, it is combined in this function.
    When dev->ops->transmit is called for each Ethernet device, the function calls this function to create
    the frame, and then executes the callback to perform processing specific to each Ethernet device
    and output.
*/
int ether_transmit_helper(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst, ssize_t (*callback)(struct net_device *dev, const uint8_t *data, size_t len)){
    ...
    // Insert padding if "len" is shorter than the minimum size
    ...
    ether_dump(frame, len);
    int ret = callback(dev, frame, flen);
    return ret == (ssize_t)flen ? 0 : -1;
}

/*
    Filter the ethernet frame that is received by executing the callback function and call net_input_handler().
    Since the frame filtering is common to all Ethernet devices, it is combined in this function.
*/
int ether_poll_helper(struct net_device *dev, ssize_t (*callback)(struct net_device *dev, uint8_t *buf, size_t size)){
    ...
    // Execute the callback function, and get the ethernet frame
    ...
    // Filtering of the ethernet frame
    hdr = (struct ether_hdr*)frame;
